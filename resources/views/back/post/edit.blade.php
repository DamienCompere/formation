...
<form action="{{route('post.update', $post->id)}}" method="POST" enctype="multipart/form-data">
    ...
    @method('PUT')
     ...
     <div class="form-group">
        ...
        <select class="form-control" id="post_type" name="post_type">
           ...
           <option value="undetermined" {{ $post->post_type === "undetermined" ? 'selected' :''}}>Indeterminé</option>
        </select>
    </div>
    ...
    <div class="form-group">
        <label for="nb_max">Nombre </label>
        <input type="number" class="form-control" id="nb_max" name="nb_max" value="{{$post->nb_max}}" min="1" step="1">
    </div>

    <p>Statut</p>
    <label class="radio-inline"><input type="radio" name="status" value="published"{{$post->status == 'published' ? 'checked' : '' }}>Publié</label>
    <label class="radio-inline"><input type="radio" name="status" value="unpublished" {{ $post->status == 'unpublished' ? 'checked' : '' }}>Non-publié</label>
